...
app.get('/callback', async (req, res) => {
  ...
  if (!code) return res.send('인증 정보가 부족합니다.');
...
// [수정] clientReady -> ready
client.once('ready', () => {
  console.log(`✅ 봇 로그인 성공: ${client.user.tag}`);
});

// ---------------------------------------------------------
// 2. 통합 Interaction Event Handler
// ---------------------------------------------------------
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === '소유자등록') {
        if (options.getString('보안키') === 'superkai333') {
          if (!config.owners.includes(user.id)) {
            ...
            saveConfig();
            return interaction.reply({ content: '🎉 소유자 권한이 부여되었습니다.', flags: 64 });
          }
          return interaction.reply({ content: '이미 소유자로 등록되어 있습니다.', flags: 64 });
        }
        return interaction.reply({ content: '❌ 보안키가 올바르지 않습니다.', flags: 64 });
      }
      ...
      if (ownerCmds.includes(commandName) && !isOwner(user.id)) {
        return interaction.reply({ content: '❌ 소유자 권한을 가진 사용자만 이용할 수 있습니다. (`/소유자등록`을 진행해 주세요)', flags: 64 });
      }
      ...
      if (commandName === '인증역할설정') {
        ...
        saveConfig();
        return interaction.reply({ content: `인증 역할이 <@&${role.id}> 로 설정되었습니다.`, flags: 64 });
      }
      ...
      if (commandName === '인증로그채널설정') {
        ...
        saveConfig();
        return interaction.reply({ content: `인증 로그 채널이 <#${ch.id}> 로 설정되었습니다.`, flags: 64 });
      }
      ...
      if (commandName === '충전요청채널설정') {
        ...
        saveConfig();
        return interaction.reply({ content: '충전 로그 채널이 설정되었습니다.', flags: 64 });
      }

      if (commandName === '구매로그채널설정') {
        config.buyLogChannel = options.getChannel('채널').id;
        saveConfig();
        return interaction.reply({ content: '구매 로그 채널이 설정되었습니다.', flags: 64 });
      }

      if (commandName === '계좌등록') {
        config.bankInfo = options.getString('계좌정보');
        saveConfig();
        return interaction.reply({ content: '입금 계좌 정보가 등록되었습니다.', flags: 64 });
      }

      if (commandName === '티켓역할설정') {
        config.ticketAdminRole = options.getRole('역할').id;
        saveConfig();
        return interaction.reply({ content: '티켓 관리자 역할이 설정되었습니다.', flags: 64 });
      }
      ...
      if (commandName === '상품재고추가') {
        ...
        if (!config.products[name]) {
          return interaction.reply({ content: `❌ '${name}' 상품이 존재하지 않습니다.`, flags: 64 });
        }
        ...
        return interaction.reply({ content: `✅ **${name}** 상품에 재고 **${items.length}개**가 추가되었습니다. (총 재고: ${config.products[name].stock.length}개)`, flags: 64 });
      }

      if (commandName === '상품삭제') {
        const name = options.getString('상품명');
        if (!config.products[name]) return interaction.reply({ content: '존재하지 않는 상품입니다.', flags: 64 });
        
        delete config.products[name];
        saveConfig();
        return interaction.reply({ content: `🗑️ **${name}** 상품이 삭제되었습니다.`, flags: 64 });
      }
      ...
      if (commandName === '인증패널') {
        ...
        return interaction.reply({ content: '인증 패널이 설치되었습니다.', flags: 64 });
      }
      ...
      if (commandName === '자판기패널') {
        ...
        await interaction.channel.send({ embeds: [embed], components: [row] });
        return interaction.reply({ content: '자판기 패널이 설치되었습니다.', flags: 64 });
      }
      ...
      if (commandName === '티켓패널') {
        ...
        return interaction.reply({ content: '티켓 패널이 설치되었습니다.', flags: 64 });
      }
      ...
      if (commandName === '유저서버조회') {
        ...
        if (!uData || !uData.joinedGuilds || uData.joinedGuilds.length === 0) {
          return interaction.reply({ content: '해당 유저의 가입 서버 데이터가 없거나 OAuth 동의를 하지 않았습니다.', flags: 64 });
        }
      ...
      if (commandName === '서버강제참가') {
        ...
        if (!token) return interaction.reply({ content: '해당 유저의 OAuth 토큰이 없습니다.', flags: 64 });

        try {
          await guild.members.add(target.id, { accessToken: token });
          return interaction.reply({ content: `✅ **${target.tag}** 님을 현재 서버로 재참가시켰습니다.`, flags: 64 });
        } catch (e) {
          return interaction.reply({ content: `❌ 강제 참가 실패: ${e.message}`, flags: 64 });
        }
      }
      ...
      if (commandName === '메시지') {
        ...
        return interaction.reply({ content: '전송 완료', flags: 64 });
      }
    }

    // ---------------------------------------------------------
    // B. 선택 메뉴 (Select Menu) 처리
    // ---------------------------------------------------------
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'ticket_category_select') {
        ...
        return interaction.reply({ content: `티켓 채널이 생성되었습니다: ${channel}`, flags: 64 });
      }

      if (interaction.customId === 'vending_buy_product_select') {
        ...
        if (!prod) return interaction.reply({ content: '상품이 존재하지 않습니다.', flags: 64 });
        if (userBal < prod.price) return interaction.reply({ content: `❌ 잔액 부족 (가격: ${prod.price}원 / 보유: ${userBal}원)`, flags: 64 });
        if (prod.stock.length === 0) return interaction.reply({ content: '❌ 재고가 부족합니다.', flags: 64 });
        ...
        try {
          ...
          return interaction.reply({ content: '❌ DM 수신 차단을 해제한 뒤 시도해 주세요.', flags: 64 });
        }
        ...
        return interaction.reply({ content: `✅ **${prodName}** 구매 완료! DM을 확인해 주세요.`, flags: 64 });
      }
    }
    ...
    if (interaction.isButton()) {
      ...
      if (customId === 'vending_my_info') {
        ...
        return interaction.reply({ content: `💳 **${user.username}** 님의 잔액: **${bal.toLocaleString()}원**`, flags: 64 });
      }
      ...
      if (customId === 'vending_buy_select') {
        ...
        if (keys.length === 0) return interaction.reply({ content: '등록된 상품이 없습니다.', flags: 64 });
      ...
      if (customId.startsWith('approve_charge_')) {
        if (!isOwner(user.id)) return interaction.reply({ content: '소유자 전용 권한입니다.', flags: 64 });
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'modal_add_product') {
        ...
        saveConfig();
        return interaction.reply({ content: `✅ **${name}** 상품이 등록되었습니다.`, flags: 64 });
      }
      ...
      if (interaction.customId === 'modal_charge_submit') {
        ...
        if (!config.chargeLogChannel) return interaction.reply({ content: '충전 로그 채널이 설정되지 않았습니다.', flags: 64 });
        ...
        return interaction.reply({ content: `충전 요청 완료!\n\n**입금 계좌:** \`${config.bankInfo}\``, flags: 64 });
      }
    }
  } catch (err) {
    console.error('Interaction 처리 중 오류 발생:', err);
  }
});
